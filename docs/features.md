...
> 版本 0.1.0 · 代码基线 `main@811fb21` · 最后更新 2026-09
...
| 项目导航 | Quick Open（Ctrl+P）、全局搜索（Ctrl+Shift+F）、右侧栏（搜索/大纲/问题/源代码） | ✅ |
...
- **头部工具栏**：全局搜索、折叠文件树、新建（`+`，根目录菜单）、**打开文件**（系统文件选择器；工作区内 → 普通可写标签，工作区外 → 只读外部标签，按路径去重、已打开则激活）、切换工作区。
...
- **Git 状态徽标**：文件树节点显示 Git 变更字母（`M` 修改 / `A` 新增 / `D` 删除 / `R` 重命名 / `U` 冲突 / `?` 未跟踪），位于文件名右侧、字号略小、使用 muted 配色；文件夹显示子节点的**代表状态**（按 M>D>A>R>U>C>? 优先级，工作区根不显示）。数据直接复用 gitStore（O(1) `Map` 查找，无新增缓存/轮询），随 Git 刷新自动更新；删除的文件不制造节点、重命名只标注新路径。
...
### 8.3 右侧栏（搜索 / 引用 / 大纲 / 问题 / 源代码）

- 顶部栏右侧按钮切换右侧栏；面板为 **搜索 / 引用 / 大纲 / 问题 / 源代码** 五标签，宽度可拖拽。
...
```json
{
  ...
  "files": {
    "autoSave": { "afterDelay": false, "onFocusChange": false, "onWindowChange": false, "delay": 1000 }
  },
  "lsp": {
    "rust": { "command": "rust-analyzer", "args": [] },
    "cpp": { "command": "clangd", "args": [] },
...
- **四区域**：左侧文件树（180–400px，默认 **220px**）、中间编辑器、底部终端（高度 ≥120px）、右侧栏（搜索/大纲/问题）均可拖拽；终端高度不超过窗口 70% 且为编辑器保留至少 160px；窗口缩放时自动收敛到合法范围。
...
- **主题**：暗色 / 亮色 / 跟随系统三态，设置在「通用 → 主题」；`system` 由 `App.tsx` 监听 `prefers-color-scheme` 实时切换，写入 `<html data-theme>`。
- **设计 Token**：全部界面颜色走 `--vo-*` Token 层（`App.css`）：dark 值等于其替代的原始十六进制，light 仅覆盖一份精选集合；Monaco 与 xterm 主题**保持独立**（各由自身配置管理）。
- **亮色分层**：窗口自 Activity Bar（`#e5e5e5`）→ 面板 chrome（`#ededed`）→ 表面/编辑器（`#f7f7f7`）形成细微灰度层次，避免大面积纯白。
- **视觉语言**：紧凑、低噪声、细边界（1px 低对比分隔）、低圆角（≤4px）、无渐变/卡片/大阴影；inactive 低对比、active 克制但明确；hover 收敛为「图标/工具栏按钮 → `#3a3d41`」与「列表/标签行 → `#2a2d2e`」两族。
- **品牌**：应用图标（`src-tauri/icons/*`，由 `pnpm tauri icon` 从 `logo.png` 生成）与自定义标题栏左侧 Logo（`public/logo.png`）同源。
...
- 非 UTF-8 文件无法打开（明确报错）；无编码选择、无换行符显示与转换。
- 标签支持拖拽排序与右键「关闭 / 关闭其他 / 关闭右侧 / 关闭全部」，但无分屏/多编辑器组。
...
- 无前端测试框架；前端以「`node` 可直接运行的 `.ts` 纯函数测试」（`gitStatusMapping` / `pathIdentity` / `diffSides` / `gitCommitTree` / `commitDiffSides` 五个 `test:*` 脚本）配合 `tsc`/`vite build` 与手工清单验证；测试覆盖主体在 Rust 侧（163 个单元测试，含 Git 状态解析、反引号解码、Diff 双侧读取、提交、提交历史与提交文件解析等用例）。

## 15. 源代码管理（Git · Phase 2）

基于系统 **Git CLI**（从 PATH 查找，不内置 git 库），提供仓库检测、更改列表查看、暂存/取消暂存、**单文件 Diff 预览**与**提交（Commit）**：

- **入口**：活动栏「源代码管理」按钮打开右侧栏「源代码」标签；已打开时再点一次收起面板。
- **仓库检测**：`git rev-parse --show-toplevel` 判定当前工作区是否位于某个仓库内；不在仓库内时面板提示「当前文件夹不是 Git 仓库」，其余功能不受影响。
- **状态读取**：`git status --short --untracked-files=all`（带 `-c core.quotepath=false`，Unicode 文件名原样显示、不做八进制转义）。结果按 **更改**（未暂存 + 未跟踪）与 **已暂存更改** 两组展示；行首字母徽标表示状态（`M` 修改 / `A` 新增 / `D` 删除 / `R` 重命名 / `U` 冲突 / `?` 未跟踪），重命名与复制行的 tooltip 显示 `旧路径 → 新路径`。路径为**工作区相对路径**、`/` 分隔（后端以 `-C <workspace>` 运行，路径解析与文件树/全局搜索一致）。
- **操作**：每个组标题栏提供「全部暂存」/「全部取消暂存」按钮；每行悬停显示「暂存」/「取消暂存」单文件按钮。落地命令为 `git add -- <path…>` 与 `git restore --staged -- <path…>`；全部暂存用 `git add -A`，全部取消暂存用 `git restore --staged -- .`（相比 `git reset` 在无 HEAD 的新仓库也不会失败）。路径一律经 `--` 传入，含空格/引号等特殊字符的文件名安全。
- **文件 Diff**：点击任一变更行（包括已删除文件）在编辑器区弹出**只读 Diff 对比视图**（Monaco `createDiffEditor`，左右分栏并排），按行所在分组取对应两侧内容 —— 后端命令 `git_diff_file(original, modified)` 用 `git show HEAD:<path>` / `git show :0:<path>` / 磁盘读取分别取回两侧 blob，再交给 DiffView 渲染；标题栏分别显示 `HEAD:<path>` / `Index:<path>` / `工作区` / `空`。两侧映射规则（纯函数 `diffSides`，见 §15.1）：
  - **已暂存更改**组：修改/删除 → `HEAD` ↔ 索引；新增 → `空` ↔ 索引；
  - **更改**组：未跟踪 → `空` ↔ 工作区；仅工作区删除（` D`）→ 索引 ↔ `空`；其余两侧都改（MM/AM…）→ 已暂存部分再改情形为 索引 ↔ 工作区，未暂存情形为 `HEAD` ↔ 工作区；
  - 重命名原始侧用**改名前的路径**取 blob；
  - blob 缺失（新增文件无 HEAD、未提交过）自动降级为 `空`，仍可正常对比；首 8KB 内含 NUL 字节的文件判定为二进制、给出「二进制文件，不支持预览」提示；`git show` 报错却非「blob 不存在」（例如 unborn HEAD 仓库的 `invalid object name 'HEAD'`）同样按空处理，不会让面板报错。
  Diff 视图是**覆盖式浮层**（不占标签、不进 modelStore/标签栏，随内容变化重建/释放 side model，URI 使用 `diff://` 命名空间以免冲突）；`Esc` 或标题栏「关闭」按钮退出；切换工作区、重新打开其它文件时旧 Diff 自动关闭。Diff 只反映**磁盘/版本库内容**，编辑器中的未保存修改不参与（需先保存）。
- **提交（Commit）**：面板底部提交框输入信息（多行，`Ctrl/Cmd+Enter` 或「提交」按钮），仅当**存在已暂存更改**且信息非空时可提交；落地命令 `git commit -m <message>`（以参数数组传值，提交信息永不会被解释为命令行参数/选项），成功后自动刷新状态与文件树；失败（无暂存更改、未配置 `user.name`/`user.email` 等）在提交框下方显示 stderr 原文。
- **自动刷新**：复用文件监听事件 `file-system-changed`，追加 **400ms** 防抖重新读取状态（编辑器保存、外部工具改动工作树文件都会触发）；每次暂存/取消暂存/提交完成后立即刷新；切换工作区时清空旧状态并重新加载。异步结果带序列号竞态保护 —— 旧工作区的迟到结果会被丢弃，不会覆盖新工作区。**文件树同步显示**：`gitStore` 每次刷新重建 `statusByPath` / `dirStatusByPath` 两个 `Map`（以 `fileKey` 绝对路径为键，复用 `pathIdentity` 处理 `\\?\`/UNC/大小写），TreeNode 按路径 O(1) 查询并展示状态字母，Stage/Unstage/Commit/外部改动后面板与文件树同时更新。
- **错误处理**：未安装 git → 面板显示 `Git executable not found`；命令非零退出 → 显示 stderr 原文；非仓库 → 提示语。暂存/刷新/提交进行中按钮保持禁用（`busy`），避免并发操作。
- **已删除文件**：状态含 `D`（列 X 或列 Y）的文件已不在磁盘上，编辑器不会去读一个不存在的文件；点击该行改为打开 **Diff**（原始侧从 `HEAD` 或索引取内容，工作区侧为 `空`），行路径以删除线+置灰标识。（这个行为修掉了此前点击删除项报 `Not a file: \\?\E:\…` 的问题 —— 报错路径只是工作区被 `canonicalize` 后的 verbatim 形态，真正原因是磁盘上已无该文件。）
- **本阶段范围外（留待后续）**：分支（Branch）、推送/拉取（Push/Pull）、冲突解决、修改/删除提示、标签与 GPG 等。Git 状态只反映**磁盘内容**，编辑器中的未保存修改不会进入 Git。

### 15.1 Diff 两侧映射（点击行 → original / modified）

| 分组 | 状态 | original（左） | modified（右） |
|---|---|---|---|
| 已暂存更改 | M / D（已暂存修改/删除） | `HEAD:<path>` | `Index:<path>`（D 时索引内容为空） |
| 已暂存更改 | A（已暂存新增） | 空 | `Index:<path>` |
| 更改 | ?（未跟踪） | 空 | 工作区（磁盘） |
| 更改 | ` D`（仅工作区删除） | `Index:<path>` | 空 |
| 更改 | M（工作区修改，未暂存） | `HEAD:<path>` | 工作区（磁盘） |
| 更改 | MM（已暂存后又改，点「更改」） | `Index:<path>` | 工作区（磁盘） |
| 更改 | AM（已暂存新增后又改，点「更改」） | `Index:<path>` | 工作区（磁盘） |
| 已暂存更改 | R（重命名） | `HEAD:<旧路径>` | `Index:<新路径>` |

「空」= 不存在的 blob，显示为空文本（新增/删除文件因此得到单侧内容的零填充对比）。

### 15.2 提交历史（Git · Phase 3.1）

源代码管理面板顶部新增 **更改 / 历史** 视图切换（仅在仓库内显示）；「历史」视图复用同一只读 Diff 浮层，把 Parent → Commit 的逐文件对比交给既有的 `DiffView`：

- **提交历史列表**：`git log -n <limit> --skip <skip> --format=%H%x1f%an%x1f%ae%x1f%ct%x1f%s%x1e`，当前分支最新在前，每页 50 条；每条显示短哈希、提交说明（subject，按 empty hash+author 丢弃垃圾行）、作者与**相对时间**（`3 小时前`，超过 30 天显示日期），tooltip 给出完整哈希与绝对时间。空仓库（unborn HEAD）返回空列表而非报错。「加载更多」追加下一页（`hasMore = 一页恰好 50 条`）。
- **提交详情 + 文件树**：点击一条提交 → `git show -M --name-status -z --format=%H%x1f%P%x1f%an%x1f%ae%x1f%ct%x1f%s%x1e <commit>`，取首个父提交（`%P`，合并提交取其第一个父；根提交无父）与变更清单（`R100\0旧路径\0新路径` 解析出状态 `M`/`A`/`D`/`R`/`C`，解析失败标 `?`；`-M` 启用重命名检测）。详情区展示短哈希、父提交、完整说明、作者/邮箱/绝对时间，以及**工作区相对路径的目录树**（`buildCommitTree`：每层目录在前、文件在后，同一目录复用节点，大小写不敏感字母序；目录默认展开，点击目录名折叠/展开；重命名/复制行 tooltip 显示 `旧路径 → 新路径`）。
- **提交 Diff**：点击树中任一文件（含删除/重命名/新增）在编辑器区弹出只读 Diff，两侧映射（纯函数 `commitDiffSides`，复用 `git_diff_file`）：
  - `M` / 未知：父提交 `旧哈希:path`（左） ↔ 提交 `新哈希:path`（右）；
  - `A`：`空`（左） ↔ 提交（右）；
  - `D`：父提交（左） ↔ `空`（右）；
  - `R` / `C`：父提交**旧路径**（左） ↔ 提交新路径（右）；
  - 标签为**短哈希+路径**（如 `a1b2c3d: src/main.cpp`），由前端 label 覆盖默认值传入。
- **后端新增**：`DiffSideRequest` 增加 `commit`（`show <commit>:<path>` 任意修订）与 `label`（标题覆盖）两字段；`diff_side_bytes` 走 `git show <commit>:<path>`，路径仍受工作区边界校验、commit 引用受 `safe_commit_ref` 校验（拒绝空串、`-`/`:` 开头、空白），blob 缺失时报错含 `does not exist`/`invalid object name` 时降级为 `空`（新增/删除文件的正常情形），非法 commit 引用则如实报错。新增 Tauri 命令 `git_log(limit, skip)` 与 `git_commit_details(commit)`。
...
23. **Git 状态与暂存**：打开一个 Git 仓库 → 活动栏「源代码管理」→ 右侧栏出现「更改 / 已暂存更改」两组；修改/删除/新建（含中文与带空格文件名）后分组与字母徽标正确；「全部暂存」→ 全部移到「已暂存更改」并把文件树徽标更新；单行悬停「暂存/取消暂存」生效；工具栏「刷新」可用；用外部编辑器（或 `git` 命令）改动文件 → 面板 400ms 内自动更新。
24. **Git 边界**：打开非仓库目录 → 面板提示非 Git 仓库；临时把 `git` 从 PATH 移除（或装到不可见处）→ 面板显示 `Git executable not found`；切换工作区后面板内容清空并加载新工作区状态。
